...
export default function selectUserPage() {
    ...
    const { data: AsignResponse, isLoading: AsigIsloading, error, postData } = usePostFetch(`http://localhost:3000/api/v1/notifications/${SelectValue == 1 ? 'user' : SelectValue == 2 ? 'course' : SelectValue ==  3 && 'all'}`)
    ...
    const AsigNot = () => {
        ...
        if (Object.keys(DatosEnviar).length < 1 && SelectValue != 3) {
            setCamposState(true)
        }else if(SelectValue == 3){// se envia la not para todos los usuarios
            postData({Id_Not: id})
        } else {
    ...
    return (
        <div className=" w-full p-4 lg:w-2/3 rounded-md shadow-md h-full flex flex-col justify-center items-center bg-white">
            ...
            <div className="w-full p-4 h-full flex-col justify-center items-center ">
                ...
                <div className={`${!VisibleData ? 'opacity-0 invisible' : 'opacity-100 visible'}  transition-opacity duration-500 ease-out w-full border-[#dfdeded0] flex lg:flex-row flex-col border-1 bg-azulSena lg:h-60 2xl:h-80 h-auto mb-3 rounded-md`}>
                    ...
                    <div className="lg:w-2/3 w-full justify-center items-center h-full flex flex-col  p-4">
                        ...
                        <div className="w-full h-40 py-4">
                            ...
                            <span className="block tracking-wide text-[rgb(219,218,218)]  text-sm mb-2">{SelectValue == 1 ? DatosMostrar.Ema_User : SelectValue == 2 && DatosMostrar ? DatosMostrar.Hor_Cont_Total : ""}</span>
